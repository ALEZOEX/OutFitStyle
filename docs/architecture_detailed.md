...
### Обзор
OutfitStyle теперь использует Istio Service Mesh для управления взаимодействием между микросервисами:
- Управление трафиком между сервисами
- Безопасность через mTLS
- Надежность через политики повторных попыток и размыкателя цепи
- Наблюдаемость через метрики, логи и трассировку

### Компоненты
- **Gateway**: Управляет внешним трафиком в кластер
- **VirtualService**: Определяет правила маршрутизации
- **DestinationRule**: Определяет политики подключения и обнаружения выбросов
- **Sidecar**: Управляет трафиком для отдельных сервисов

## Event-driven архитектура

### Обзор
Для асинхронной обработки рекомендаций реализована event-driven архитектура:
- Используется Apache Kafka в качестве брокера сообщений
- API-сервис публикует события запросов рекомендаций
- ML-сервис потребляет события и публикует результаты
...
- `UserFeedbackEvent`: Обратная связь пользователя о рекомендациях

### Поток событий
1. Пользователь запрашивает рекомендации через API
2. API-сервис публикует `RecommendationRequestedEvent` в Kafka
3. ML-сервис потребляет событие и обрабатывает рекомендации
4. ML-сервис публикует `RecommendationProcessedEvent` с результатами
5. API-сервис получает результаты и возвращает пользователю
...
- Надежные фоллбеки
- Service Mesh для управления сервисами
