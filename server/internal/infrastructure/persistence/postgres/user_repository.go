...
func (r *UserRepository) GetUser(ctx context.Context, userID int) (*domain.User, error) {
	query := `
		SELECT id, email, username, avatar_url, created_at, updated_at, is_verified
		FROM users
		WHERE id = $1
	`
...
func (r *UserRepository) GetUserByEmail(ctx context.Context, email string) (*domain.User, error) {
	query := `
		SELECT id, email, username, password, avatar_url, created_at, updated_at, is_verified
		FROM users
		WHERE email = $1
	`
...
func (r *UserRepository) CreateUser(ctx context.Context, user *domain.User) error {
	query := `
		INSERT INTO users (email, username, password, avatar_url, is_verified, created_at, updated_at)
	...
	r.logger.Info("Created new user", zap.Int("user_id", user.ID), zap.String("email", user.Email))
	return nil
}
...
func (r *UserRepository) UpdateUser(ctx context.Context, user *domain.User) error {
	query := `
		UPDATE users
		SET email = $1, username = $2, password = $3, avatar_url = $4, is_verified = $5, updated_at = $6
		WHERE id = $7
	`
	...
	r.logger.Info("Updated user", zap.Int("user_id", user.ID))
	return nil
}
...
func (r *UserRepository) GetUserStats(ctx context.Context, userID int) (*domain.UserStats, error) {
	query := `
		SELECT total_recommendations, average_rating, favorite_count, achievement_count, last_active, most_used_category
		FROM user_stats
		WHERE user_id = $1
	`
...
func (r *UserRepository) UpdateUserStats(ctx context.Context, userID int, stats *domain.UserStats) error {
	query := `
		UPDATE user_stats
		SET total_recommendations = $1, average_rating = $2, favorite_count = $3,
		    achievement_count = $4, last_active = $5, most_used_category = $6
		WHERE user_id = $7
	`
