...
func (h *RecommendationHandler) GetRecommendationHistory(w http.ResponseWriter, r *http.Request) {
	...
	// Получаем историю рекомендаций пользователя
	qp := r.URL.Query()
	var page, limit int
	if p := qp.Get("page"); p != "" {
		if pageNum, err := strconv.Atoi(p); err == nil && pageNum > 0 {
	...
	list, total, err := h.svc.List(r.Context(), userID, q)
	if err != nil {
		resp.Error(w, http.StatusInternalServerError, errors.New("failed to list history"))
		return
	}
...
func (h *RecommendationHandler) RemoveFavorite(w http.ResponseWriter, r *http.Request) {
	...
	// Удаляем из избранного (устанавливаем IsFavorite = false)
	if err := h.svc.SetFavorite(r.Context(), userID, id, false); err != nil {
		resp.Error(w, http.StatusBadRequest, err)
		return
	}
	resp.Success(w, map[string]any{"success": true})
