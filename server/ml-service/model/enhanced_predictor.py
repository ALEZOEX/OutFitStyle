...
class EnhancedOutfitPredictor:
    ...
    def build_outfit(
        ...
        logger.info(
            f"Items by category: { {k: len(v) for k, v in items_by_category.items()} }"
        )
        ...
        for category in required_categories:
            ...
            if not filtered_items:
                logger.debug(
                    f"No filtered items for {category}, using all items in category"
                )
                filtered_items = items_in_category  # Fallback

            logger.debug(
                f"Items in category {category}: {len(items_in_category)}, filtered: {len(filtered_items)}"
            )
            ...
            if category_recommendations:
                ...
                logger.debug(
                    f"Selected for {category}: {best_item['name']} "
                ...
                if fallback_recommendations:
                    ...
                    category_confidences[category] = fallback_recommendations[0][
                        "ml_score"
                    ]
                    logger.debug(
                        f"Fallback selected for {category}: {fallback_recommendations[0]['name']}"
                    )
        ...
        if should_add_accessories and "accessories" in items_by_category:
            accessories = self._select_accessories(
                ...
                min_confidence=0.3,  # Lower threshold for accessories
            )
        ...
        if outfit_items:
            ...
            base_score = sum(item["ml_score"] for item in outfit_items) / len(
                outfit_items
            )
        ...
        logger.info(
            f"Built outfit: {len(outfit_items)} items, "
    ...
    def _get_warmth_level(self, item: Dict, default: float = 5.0) -> float:
        """
        Безопасно получает warmth_level как число (float).
        Обрабатывает ситуации, когда из БД/JSON приходит строка.
        """
        ...
        try:
            ...
            logger.debug(
                f"Cannot parse warmth_level={raw!r} for item {item.get('name')}, "
                f"using default={default}"
            )
            return float(default)

    def _get_temp_value(self, item: Dict, key: str, default: float) -> float:
        """
        ...
        try:
            ...
            logger.debug(
                f"Cannot parse {key}={raw!r} for item {item.get('name')}, "
                f"using default={default}"
            )
            return float(default)
    ...
    def _filter_by_user_profile(
        self, items: List[Dict], user_profile: Dict, category: str
    ) -> List[Dict]:
        ...
        for item in items:
            ...
            warmth = self._get_warmth_level(item, default=5)
        ...
        logger.debug(
            f"Filtered {len(filtered)}/{len(items)} items for {category} "
    ...
    def _should_add_accessories(
        self, temperature: float, weather: str, user_profile: Dict
    ) -> bool:
    ...
    def _select_accessories(
        ...
        logger.debug(
            f"Selected {len(selected)} accessories for temp={temp}°C, weather={weather}"
        )
    ...
    def recommend_items(
        ...
        if not self.model.is_trained:
            ...
            return self._fallback_recommendations(
                weather_data, available_items, top_n
            )

        logger.info(
            f"Generating ML recommendations for {len(available_items)} items..."
        )
        ...
        try:
            ...
            if recommendations:
                avg_conf = sum(item["ml_score"] for item in recommendations) / len(
                    recommendations
                )
                logger.info(
                    f"Generated {len(recommendations)} ML-powered recommendations"
                )
                logger.info(f"Average confidence: {avg_conf:.2%}")
            else:
                max_prob = max(probabilities) if probabilities.size > 0 else 0
                logger.info(
                    f"No recommendations above threshold. "
                    f"Max probability: {max_prob:.2%}, threshold: {min_confidence:.2%}"
                )
    ...
    def _prepare_weather_features(self, weather_data: Dict) -> Dict:
        ...
        return {
            ...
            "feels_like": weather_data.get(
                "feels_like", weather_data.get("temperature", 20)
            ),
    ...
    def _prepare_user_features(self, user_profile: Dict) -> Dict:
        ...
        return {
            ...
            "temperature_sensitivity": user_profile.get(
                "temperature_sensitivity", "normal"
            ),
            "formality_preference": user_profile.get(
    ...
    def _prepare_item_features(self, item: Dict) -> Dict:
        ...
        return {
            ...
            "min_temp": self._get_temp_value(item, "min_temp", 0),
            "max_temp": self._get_temp_value(item, "max_temp", 30),
            "warmth_level": self._get_warmth_level(item, default=5),
    ...
    def _group_by_category(self, items: List[Dict]) -> Dict[str, List[Dict]]:
        ...
        logger.debug(
            f"Grouped items by category: { {k: len(v) for k, v in grouped.items()} }"
        )
        return grouped
    ...
    def _fallback_recommendations(
        self, weather_data: Dict, items: List[Dict], top_n: int
    ) -> List[Dict]:
        ...
        scored_items = []
        for item in items:
            # Безопасно приводим min_temp / max_temp к числу
            min_temp = self._get_temp_value(item, "min_temp", 0)
            max_temp = self._get_temp_value(item, "max_temp", 30)
            ...
            if "дождь" in weather or "rain" in weather:
                ...
                warmth = self._get_warmth_level(item, default=0)
                if warmth > 6:
                    score += 0.1
            elif temp > 25:
                # Prefer lighter items in hot weather
                warmth = self._get_warmth_level(item, default=10)
                if warmth < 4:
                    score += 0.1
        ...
        scored_items.sort(key=lambda x: x["ml_score"], reverse=True)
        logger.info(
            f"Fallback recommendations generated: {len(scored_items)} items, "
            f"top score: {scored_items[0]['ml_score'] if scored_items else 0:.2%}"
        )
