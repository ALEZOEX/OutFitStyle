...
def main():
    ...
    # Удалим предыдущие импортированные данные из Kaggle (с пометкой source='kaggle_seed')
    cur.execute("DELETE FROM clothing_items WHERE source = 'kaggle_seed'")
    logger.info("Cleared previous Kaggle data")
    ...
    cur.close()
    conn.close()
    logger.info(f"✅ Done. Inserted total {inserted} rows into clothing_items from Kaggle dataset")
