...
class RecommendationService {
    ...
    return items.where((item) {
      ...
      if (condition == 'snowy' || condition == 'snow') {
        if (item.snowOk != true && item.category != 'top' && item.category != 'bottom') {
          if (item.category == 'shoes' || item.category == 'outerwear') {
            if (item.snowOk != true && (item.warmthLevel ?? 0) < 3) return false;
          }
        }
      }
  ...
  List<WardrobeItem> _applyPreferences(List<WardrobeItem> items, UserPreference preferences) {
    var filtered = items;
    
    // Фильтр по стилям
    final preferredStyles = preferences.preferredStyles;
    if (preferredStyles.isNotEmpty) {
      final styleFiltered = filtered.where((item) {
        final itemStyle = item.style?.toLowerCase() ?? '';
        return preferredStyles.any((style) => 
          itemStyle.contains(style.toLowerCase()) || style.toLowerCase().contains(itemStyle)
        );
      }).toList();
    ...
    if (preferredColors.isNotEmpty) {
      final colorFiltered = filtered.where((item) {
        final itemColor = item.color?.toLowerCase() ?? '';
        return preferredColors.any((color) => 
          itemColor.contains(color.toLowerCase()) || color.toLowerCase().contains(itemColor)
        );
      }).toList();
    ...
    if (preferredBrands.isNotEmpty) {
      final brandFiltered = filtered.where((item) {
        final itemBrand = item.brand?.toLowerCase() ?? '';
        return preferredBrands.any((brand) => 
          itemBrand.contains(brand.toLowerCase()) || brand.toLowerCase().contains(itemBrand)
        );
      }).toList();
    ...
    if (excludedItems.isNotEmpty) {
      filtered = filtered.where((item) {
        ...
        return !excludedItems.any((excluded) => 
          itemId == excluded || itemName.contains(excluded.toLowerCase())
        );
      }).toList();
    }
  ...
  List<WardrobeItem> _selectOutfitItems(
    ...
    final categories = ['top', 'bottom', 'outerwear', 'shoes', 'headwear', 'accessory'];
  ...
  String _generateDescription(
    ...
    final outerwear = outfitItems.where((item) => item.category == 'outerwear').toList();
    if (outerwear.isNotEmpty && temperature < 15) {
      parts.add('Верхняя одежда поможет сохранить тепло');
    }
    
    // Аксессуары
    final accessories = outfitItems.where((item) => item.category == 'accessory' || item.category == 'headwear').toList();
